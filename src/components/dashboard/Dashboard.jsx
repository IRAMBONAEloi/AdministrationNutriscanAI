...
export const Dashboard = () => {
  ...
  const updateOrderStatus = async (order, newStatus) => {
    try {
      ...
      if (response.data?.success) {
        ...
        // Emit status update via socket for real-time sync
  ...
  const deleteOrder = async (orderId) => {
    try {
      ...
      if (response.data?.success) {
        ...
        if (socket && socket.connected) {
          socket.emit('order_deleted', orderId);
        }
      } else {
        toast.error(response.data?.message || "Delete failed");
      }
  ...
  useEffect(() => {
    ...
    // Tell the backend to put this screen in the 'management' room
    newSocket.emit('join_room', 'management');

    // Listen for brand new orders
    newSocket.on('new_order_alert', (newOrder) => {
      ...
      // Show notification
      const tableNumber = newOrder.personDetails?.tableNumber || "Unknown";
      setNotification(`⚠️ New Order placed for Table ${tableNumber}!`);
      toast.info(`New order received! Table ${tableNumber}`);
    ...
    // Listen for status updates from the kitchen (order_ready_alert)
    newSocket.on('order_ready_alert', (updatedOrder) => {
      setNotification(`✅ Table ${updatedOrder.personDetails?.tableNumber}'s meal is ready to serve!`);
      toast.success(`Order #${updatedOrder.orderId?.slice(-8)} is ready!`);
      
      // Update the status of the item inside your existing state array
      setOrders((prevOrders) =>
    ...
    newSocket.on('order_status_updated', (updatedOrder) => {
      ...
      toast.info(`Order #${updatedOrder.orderId?.slice(-8)} status updated to ${updatedOrder.status}`);
    ...
    newSocket.on('order_deleted', (deletedOrderId) => {
      setOrders((prevOrders) => prevOrders.filter(order => order._id !== deletedOrderId));
      toast.warning(`An order has been removed from the system`);
      fetchAnalytics();
    });

    // Listen for preparation started events
    newSocket.on('preparation_started', (data) => {
      toast.info(`Chef started preparing order #${data.orderId?.slice(-8)}`);
    });

    // Listen for connection events
    newSocket.on('connect', () => {
      console.log('Socket.IO connected to management room');
      toast.success('Connected to real-time server');
    });

    newSocket.on('disconnect', () => {
      console.log('Socket.IO disconnected');
      toast.warning('Real-time connection lost. Updates may be delayed.');
    });
    ...
    // CLEANUP: Disconnect if the user leaves this page/component unmounts
    return () => {
      if (newSocket) {
    ...
    >
      ...
      <AnimatePresence>
        {notification && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -50 }}
            className="fixed top-20 left-1/2 transform -translate-x-1/2 z-50 bg-gradient-to-r from-orange-500 to-red-500 text-white px-6 py-3 rounded-full shadow-lg flex items-center gap-3"
          >
            <motion.div
            ...
            <button onClick={() => setNotification("")} className="ml-2 hover:bg-white/20 rounded-full p-1">
      ...
      >
        ...
        <div className="flex items-center gap-2">
          ...
          <motion.div
            animate={{ scale: socket?.connected ? [1, 1.2, 1] : 1 }}
            transition={{ duration: 2, repeat: socket?.connected ? Infinity : 0 }}
