...
export const ChefDashboard = () => {
  ...
  const startPreparation = async (order) => {
    ...
    if (socket && socket.connected) {
      socket.emit('preparation_started', {
        orderId: order._id,
        chef: localStorage.getItem("user") || "Unknown Chef",
        timestamp: new Date()
      });
    }
  };
  ...
  useEffect(() => {
    ...
    // Tell the backend to put this screen in the 'kitchen' room
    newSocket.emit('join_room', 'kitchen');

    // Listen for database insertions in real-time
    newSocket.on('new_order_alert', (newOrder) => {
      // Transform the new order to match our component's order structure
      const transformAndAddOrder = async () => {
        try {
          const menuMap = await fetchMenuItems();
          const transformedOrder = await transformOrder(newOrder, menuMap);
          
          // Add the new order to the existing orders list
          setOrders((prevOrders) => {
            // Check if order already exists to avoid duplicates
            const exists = prevOrders.some(order => order._id === transformedOrder._id);
            if (exists) {
              // Update existing order instead of adding duplicate
              return prevOrders.map(order => 
                order._id === transformedOrder._id ? transformedOrder : order
              );
            }
          ...
          toast.info(`New order received! Table ${transformedOrder.tableNumber}`);
        } catch (error) {
          console.error("Error transforming real-time order:", error);
        }
      };
      
      transformAndAddOrder();
    });

    // Listen for order status updates from other clients
    newSocket.on('order_status_updated', (updatedOrderData) => {
      const updateExistingOrder = async () => {
        try {
          const menuMap = await fetchMenuItems();
          const transformedOrder = await transformOrder(updatedOrderData, menuMap);
          
          setOrders((prevOrders) => {
            const orderExists = prevOrders.some(order => order._id === transformedOrder._id);
            if (orderExists) {
              return prevOrders.map(order => 
                order._id === transformedOrder._id ? transformedOrder : order
              );
          ...
          toast.info(`Order #${transformedOrder.orderId?.slice(-8)} status updated to ${transformedOrder.status}`);
          
          // Recalculate stats
          calculateStats(orders);
        } catch (error) {
          console.error("Error updating order from socket:", error);
        }
      };
    ...
    newSocket.on('order_deleted', (deletedOrderId) => {
      setOrders((prevOrders) => {
        const updatedOrders = prevOrders.filter(order => order._id !== deletedOrderId);
    ...
    newSocket.on('connect', () => {
      console.log('Socket.IO connected to kitchen room');
      setIsConnected(true);
      toast.success('Connected to real-time kitchen server');
    });
